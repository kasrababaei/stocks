...
final class StocksListViewModelTests {
  ...
  func loadFirstPage() async {
    let stocks = Stock.mockStocks(count: 300)

    mockService.stockReturnCall = { stocks }
    let viewModel = StocksListViewModel()

    var objectDidChange: Bool { objectWillChangeCount > 0 }
    var objectWillChangeCount = 0
    viewModel.objectWillChange.sink { objectWillChangeCount += 1 }.store(in: &cancellables)

    await viewModel.loadData()

    #expect(objectDidChange)
